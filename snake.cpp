...
void Snake::Draw() {
  ...
  for (int i = 0; i < (int)body.size(); i++) {
    ...
    Rectangle segment;
    segment.x = offset + cellPos.x * cellSize;
    segment.y = offset + cellPos.y * cellSize;
    segment.width = (float)cellSize;
    segment.height = (float)cellSize;

    DrawRectangleRounded(segment, 0.5, 6, darkGreen);

    // Eyes on head (optional): keep them attached to the interpolated head
    // rectangle
    if (i == 0) {
      ...
      Vector2 center = (Vector2){segment.x + segment.width * 0.5f,
                                 segment.y + segment.height * 0.5f};
      Vector2 perp = (Vector2){-direction.y, direction.x};

      Vector2 leftEye =
          Vector2Add(center, Vector2Add(Vector2Scale(perp, inset),
                                        Vector2Scale(direction, forward)));
      Vector2 rightEye =
          Vector2Add(center, Vector2Add(Vector2Scale(perp, -inset),
                                        Vector2Scale(direction, forward)));
...
void Snake::reset() {
  ...
  prevBody = body;
  // We use a C-style cast (Vector2){1,0} because it is explicit and
  // compatible with both C++ and C-style struct initialization.
